...
namespace AstrolabeDC.dcSharp
{
    internal class GallDetail
    {
        ...
        public async Task GetGallDetail(string URL)
        {
            ...
            httpClient.Timeout = TimeSpan.FromMinutes(2) + TimeSpan.FromSeconds(30);
        ...
        public async Task<List<IDictionary<string, string>>> DetailData()
        {
            ...
            modifiedHtml = Regex.Replace(modifiedHtml, @"<div\b[^>]*\bclass\s*=\s*['""][^'""]*og-div[^'""]*['""][^>]*>(.*?)</div>", "&#&<div class='og-div'>$1</div>&#&");
            modifiedHtml = Regex.Replace(modifiedHtml, @"<div\b[^>]*\bclass\s*=\s*['""][^'""]*og-info[^'""]*['""][^>]*>(.*?)</div>", "");
            modifiedHtml = Regex.Replace(modifiedHtml, @"<div\b[^>]*\bclass\s*=\s*['""][^'""]*og-inr[^'""]*['""][^>]*>(.*?)</div>", "");
            modifiedHtml = Regex.Replace(modifiedHtml, @"<p\b[^>]*\bclass\s*=\s*['""][^'""]*og-txt[^'""]*['""][^>]*>(.*?)</p>", "");
            ...
            foreach (var item in stringList)
            {
                ...
                var linkBox = doc.DocumentNode.SelectNodes("//div/div[@class='og-div']/a[@href]");
                ...
                {
                    foreach (HtmlNode attLinkBox in linkBox)
                    {
                        HtmlAttribute attLink = attLinkBox.Attributes["href"];
