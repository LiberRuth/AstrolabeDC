...
namespace AstrolabeDC.dcSharp
{
    internal class GallList
    {
        ...
        public async Task GetGallList(string URL)
        {
            ...
            httpClient.Timeout = TimeSpan.FromMinutes(3) + TimeSpan.FromSeconds(30);
            try
            {
        ...
        public List<IDictionary<string, string>> Gall_list()
        {
            ...
            foreach (var Nodes in htmlNodes_list!)
            {
                ...
                var gallSubject_inner = Nodes.SelectNodes(".//td[@class='gall_subject']/p[@class='subject_inner']");
                var gallSubject = Nodes.SelectNodes(".//td[@class='gall_subject']");

                if (gallSubject_inner != null)
                {
                    foreach (var subject in gallSubject_inner)
                    {
